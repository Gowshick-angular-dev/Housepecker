...
class PropertyRepository {
  ...
  Future<DataOutput<PropertyModel>> fetchProperty({required int offset}) async {
    ...
    List<PropertyModel> modelList = (response['data'] as List)
        .where((e) => e['is_type'] == 'property').toList()
  ...
  Future<DataOutput<PropertyModel>> fetchNearByProperty(
    ...
    try {
      ...
      List<PropertyModel> dataList = (result['data'] as List)
          .where((e) => e['is_type'] == 'property').toList()
          .map((e) {
  ...
  Future<DataOutput<PropertyModel>> searchProperty(String searchQuery,
    ...
    List<PropertyModel> modelList = (response['data'] as List)
        .where((e) => e['is_type'] == 'property').toList()
        .map((g) => PropertyModel.fromMap(g)).toList();

    List modelList2 = (response['data'] as List)
        .where((e) => e['is_type'] == 'project').toList();

    return DataOutput(total: response['total'] ?? 0, modelList: modelList, modelList2: modelList2 ?? []);
  }
  ...
  Future<DataOutput<PropertyModel>> fetchProperyFromType(
      {required String type, required int offset, bool? showPropertyType}) async {
    Map<String, dynamic> parameters = {
      Api.propertyType: type == 'buy' ? 0 : 1,
      Api.offset: offset,
      Api.limit: Constant.loadLimit,
      "current_user": HiveUtils.getUserId()
    };
  ...
  Future<PropertyModel> fetchBySlug(String slug) async {
    ...
    return PropertyModel.fromMap(result['data'][0].where((e) => e['is_type'] == 'property').toList());
  }
